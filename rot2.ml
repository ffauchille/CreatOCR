...
(*	if ((r=0 && g=0 && b=0)&&(r1=255 && g1=255 && b1= 255)&&(r2=255 && g2=255 && b2= 255)&&(r3=255 && g3=255 && b3= 255)&&(r4=255 && g4=255 && b4= 255)) then*)
	if (r=0 && r1=255 && r2=255 && r3=255 && r4=255)then 
	  begin
...
let rotation90 src dst =
  let tab = rot90 src dst in
  let (w,h)= get_dims src in
  begin
    for i=0 to 899 do
      for h =0 to 899 do
	Sdlvideo.put_pixel_color dst i h (255,255,255);
      done;
    done;
    for x=0 to h-1 do
      for y=0 to w-1 do
	Sdlvideo.put_pixel_color dst (x+150) (y+150) tab.(x).(y); 
      done;
    done;
  end
...
let rotation270 src dst =
  let tab = rot270 src dst in
  let (w,h)= get_dims src in
  begin
    for i=0 to 899 do
      for h =0 to 899 do
...
let rot_auto img dst=
  begin
    let xu = ref 0 in(*coord du 1er pixel haut*)
    let yu = ref 0 in
    let xd = ref 0 in(*coord du plus bas*)
    let yd = ref 0 in
    let ab = ref 0. in (*distance entre les deux points*)
    let ac = ref 0. in (*distance du coté adjacent*) 
    let alpha = ref 0. in (*valeur de l'angle de rotation*)
    let pi = 4.4 *. atan 1.0 in(* pi *)
    from_down_left img xd yd; (* acquiert les coord des deux points*)
    from_up_left img xu yu;
